...
public class ConcurrentImageProcessorDataParallel {
	private MyThread reader, processor, writer;
	...
	public ConcurrentImageProcessorDataParallel(String f, File td, long ot, int threads) {
		showProgressBar();
		semaphore = new CIPSemaphore();
		rpList = new ArrayList<BufferedImage>();
		pwList = new ArrayList<BufferedImage>();
	    reader = new MyThread(semaphore, null, rpList, f, td, ot, 0, threads); 
	    processor = new MyThread(semaphore, rpList, pwList, f, td, ot, 1, threads);
	    writer = new MyThread(semaphore, pwList, null, f, td, ot, 2, threads);
	    reader.start();
	    processor.start();
	    writer.start();
	...
	private class MyThread extends Thread {
		...
		private int type, threads;
		long cReadTime, readTime, cProcessTime, processTime, cWriteTime, writeTime, overallTime;

		public MyThread(CIPSemaphore s, ArrayList<BufferedImage> il, ArrayList<BufferedImage> ol, String f, File td, long ot, int t, int threads) {
			semaphore = s;
			inList = il;
			outList = ol;
			filter = f;
				...
				if (type == 0) {
					...
					switch (filter) {
						...
						case "invert":
							...
							break;
						case "weird":
							imageFilter = new DataParallelWeirdFilter(threads);
							break;
					}
	...
	public static void main(String args[]) {
		...
		if (args.length == 2) {
			...
			System.err.println("Usage: java ConcurrentImageProcessorDataParallel [# of threads] <filter> <list of filenames>");
			System.exit(1);
		}
		...
		if (dir.isDirectory() && Arrays.asList(FILTERS).contains(filter)) {
			final ConcurrentImageProcessorDataParallel cip = new ConcurrentImageProcessorDataParallel(args[1], dir, overallTime, threads);
